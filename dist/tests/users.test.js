...
describe('User endpoint', () => {
  before(_asyncToGenerator(function* () {
    ...
    testUser2 = yield _userModel2.default.create(_userFactory2.default.generate());
  }));
  ...
  describe('User login', () => {
    it('should find a user with login', done => {
      _serverMock2.default.post(`${ENDPOINT}/login`).send({ username: testUser2.username, password: 'password1' }).end((err, res) => {
        const body = res.body,
              status = res.status;

        (0, _chai.expect)(status).to.equal(201);
        (0, _chai.expect)(body.username).to.equal(testUser2.username);
        (0, _chai.expect)(body).to.haveOwnProperty('_id');
        done();
      });
    });

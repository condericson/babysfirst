...
describe('firsts actions', () => {
  it('#getFirstsSuccess(): should get a lists of firsts', () => {
    expect(
      firstsReducer(undefined, getFirstsSuccess(dummyData.firsts1)),
    ).toMatchSnapshot();
  });
  it('#loadMoreSuccess(): should get another list of firsts', () => {
    const state = firstsReducer(undefined, getFirstsSuccess(dummyData.firsts1));
    expect(state).toMatchSnapshot();
    expect(
      firstsReducer(state, loadMoreSuccess(dummyData.firsts2)),
    ).toMatchSnapshot();
  });
  ...
  it('#addFirstsSuccess(): should add a first to list of firsts', () => {
    ...
    expect(state).toMatchSnapshot();
    expect(
      firstsReducer(state, addFirstsSuccess(dummyData.firsts3)),
    ).toMatchSnapshot();
  });
  ...
  it('#deleteFirstsSuccess(): should add a first to list of firsts', () => {
    ...
    expect(state).toMatchSnapshot();
    expect(
      firstsReducer(state, deleteFirstsSuccess(dummyData.firsts1)),
    ).toMatchSnapshot();
  });
});
